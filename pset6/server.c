...
char* indexes(const char* path)
{
    char* dothtml="index.html";
    char* dotphp="index.php";
    //string for index.html
    char* php=malloc(strlen(path)+strlen("index.php")+1);
    //string for index.html
    char* html=malloc(strlen(path)+strlen("index.html")+1);
    //copying from path to php
    strcpy(php,path);
    //copying from path to html
     strcpy(html,path);
     //concatenation
     strcat(html,dothtml);
     strcat(php,dotphp);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    //checking for non-availability of memory
    if(file==NULL)
    {
        return false;
    }
    //counter for number of bytes
    int counter=0;
   
    //where file will be written contiguously
    char* buffer=malloc(sizeof(BYTE));

    //start reading from the file
    while(fread((buffer+counter),1,1,file)>=1)
    {
        
        //read from the file into buffer
        counter++;
        //re allocating the memory for buffer
        buffer=realloc(buffer,counter+1);
    }
    //dereferencing the length
    *length=counter;
    //derefernce content equals to buffer
    *content=buffer;
...
const char* lookup(const char* path)
{
    ...
    char* mtype=NULL;
    char ch='.';
     const char* ft=strchr(path,ch);
    if(strcasecmp(".css",ft)==0)
    {
        mtype="text/css";
        return mtype;
    }
    else if(strcasecmp(".html",ft)==0)
    {
        mtype="text/html";
        return mtype;
    }
    else if(strcasecmp(".gif",ft)==0)
    {
        mtype="image/gif";
        return mtype;
    }
    else if(strcasecmp(".ico",ft)==0)
    {
        mtype="image/x-icon";
        return mtype;
    }
    else if(strcasecmp(".jpg",ft)==0)
    {
        mtype="image/jpeg";
        return mtype;
    }
    else if(strcasecmp(".js",ft)==0)
    {
        mtype="text/javascript";
        return mtype;
    }
    else if(strcasecmp(".php",ft)==0)
    {
        mtype="text/x-php";
        return mtype;
    }
    else if(strcasecmp(".png",ft)==0)
    {
        mtype="image/png";
        return mtype;
    }
...
{
    //pointer to be used for extraction
  char* ptr=NULL;
  
  //method pointer
  char* method = malloc((strlen(line)*sizeof(char))+2);
  //pointer copied to method
  strcpy(method,line);
  //searching for first space
  ptr=strchr(method,' ');
  //same steps followed for each extraction
  char* requestline=malloc(strlen(ptr)*sizeof(char));
  //putting deliminator for separation
  ptr[0]='\0';
  
  
  strcpy(requestline,ptr+1);
  
  ptr=strchr(requestline,' ');
   char* http=malloc((strlen(ptr)*sizeof(char))+2);
  ...
  ptr=strchr(http,'\r');
  ptr[0]='\0';
  //checking for query in line
  if((ptr=(strchr(requestline,'?')))!=NULL)
  {
      if(strchr(requestline,'q')!=NULL)
      {
          ptr[0]='\0';
          strcpy(query,ptr+1);
          query[strlen(query)]='\0';
          strcpy(abs_path,requestline);
      }
      else
      {
          ptr[0]='\0';
 ...
 *
 * http://www.w3.org/Protocols/rfc26
 * 16/rfc2616-sec6.html#sec6
